...
private:
    ...
    static constexpr size_t COMPRESSED_SIZE = COORDINATE_SIZE + 1;

    /**
     * Get the parity bit of a field element (LSB of the coordinate)
     */
    uint8_t getParityBit(const typename Engine::FqElement& element) const {
     ...
     * For Alt-BN128: y² = x³ + 3
     */
    void decompressYCoordinate(Point& point, uint8_t parity) const {
        typename Engine::FqElement x_cubed, y_squared, y;

        // Compute x³
        engine.f1.square(x_cubed, point.x);
        engine.f1.mul(x_cubed, x_cubed, point.x);

        // Add curve parameter b (3 for Alt-BN128)
        typename Engine::FqElement three;
        engine.f1.fromString(three, "3");
        engine.f1.add(y_squared, x_cubed, three);

        // Compute square root
        if (!engine.f1.sqrt(y, y_squared)) {
            // Point is not on curve - this shouldn't happen with valid data
            throw std::runtime_error("Invalid compressed point: not on curve");
        }
...
public:
    ...
    * Constructor for zkey point storage
     ...
     */
    ZKopPointStorage(const void* raw_data, uint64_t data_size, Engine& eng) : zeroTracker(*static_cast<const uint32_t*>(raw_data)), engine(eng)  {
