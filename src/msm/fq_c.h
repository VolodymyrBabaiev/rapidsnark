/**
 * fq.h - Field element operations for the Fq field
...
// Set functions equivalent to C++ class methods
void Fq_set_zero(FqElement* e);
void Fq_set_one(FqElement* e);
void Fq_set_neg_one(FqElement* e);
void Fq_set(FqElement* e, int value);
int Fq_get_bytes_size();
int Fq_to_rpr_be(const FqElement* element, uint8_t* data, int bytes);
int Fq_from_rpr_be(FqElement* element, const uint8_t* data, int bytes);
void Fq_from_ui(FqElement* e, unsigned long int value);
void Fq_from_string(FqElement* e, const char* str, int radix);
char* Fq_to_string(const FqElement* a, int radix);
