...
uint64_t MSMmem<Curve, BaseField>::getBitsPerChunk(uint64_t n, uint64_t scalarSize) const
{
...
template <typename Curve, typename BaseField>
void MSMmem<Curve, BaseField>::run(typename Curve::Point &r,
                                PointStorageInterface<typename Curve::PointAffine> *_bases,
                                uint8_t* _scalars,
                                ...
                                uint64_t _nThreads)
{
    ...
    if (nPoints == 1) {
		typename Curve::PointAffine p = _bases->get(0);
        g.mulByScalar(r, p, _scalars, _scalarSize);
        return;
    }
    ...
    threadPool.parallelFor(0, nChunks, [&] (int64_t begin, int64_t end, uint64_t idThread) {
        ...
        for (int64_t j = begin; j < end; j++) {
            ...
            for (int64_t i = 0; i < nPoints; i++) {
                ...
                if (bucketIndex > 0) {
                    typename Curve::PointAffine p = _bases->get(i);
                    g.add(buckets[bucketIndex-1], buckets[bucketIndex-1], p);

                } else if (bucketIndex < 0) {
                    typename Curve::PointAffine p = _bases->get(i);
                    g.sub(buckets[-bucketIndex-1], buckets[-bucketIndex-1], p);
                }
            }
